...
async def main():
    os.makedirs(os.path.join(OUT, "public", "images", "og"), exist_ok=True)
    os.makedirs(os.path.join(OUT, "public", "images", "icons"), exist_ok=True)
    async with async_playwright() as p:
        b = await p.chromium.launch()
        pg = await b.new_page(viewport={"width": 1200, "height": 630})
        await pg.set_content(html_og())
        await pg.wait_for_timeout(150)
        await pg.screenshot(path=os.path.join(OUT, "public", "images", "og", "og-default.jpg"),
                            type="jpeg", quality=88)
        pg = await b.new_page(viewport={"width": 64, "height": 64})
        await pg.set_content(html_fav())
        await pg.wait_for_timeout(100)
        await pg.screenshot(path=os.path.join(OUT, "public", "images", "icons", "favicon.png"))
        await b.close()

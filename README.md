...
Оригинал отправлял их на свой сервер (`/sendform`). На статике такого нет.
Сейчас в `config.json` → `leads.mode` стоит **`supabase`**: заявка уходит напрямую
в хранилище Supabase из браузера (публичный anon-ключ, секретов в коде нет).
Веб-админка читает заявки через серверную функцию с паролем.
